...
const Admins = () => {
    ...
    const addNewUser = async (user: any, image : File | null) => {
        ...
        const res = await fetchData("/user/registerAdmin", setFormSubmitLoading, "POST", formData);
        if (res?.success) {
    ...
    const handleDownloadReport = () => {
        const csv = [
            ...
            ...data.map((d: any) => [d?._id,d.name, d.email, d.contactNumber])
        ].map(e => e.join(",")).join("\n");
...

            {
                loading ? (
                    ...
                    !showForm ? (
                        ...
                        <Form
                            title={editForm ? "Update Staff Details" : "Add New Staff"}
                            isEditing={editForm}
