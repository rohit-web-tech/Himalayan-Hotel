...
const Contact = () => {
    ...
    const getData = async () => {
        const data = await fetchGetData("/contact", setLoading);
        console.log(data)
        if (data?.success) {
            setData(data?.data || {
                title: "",
    ...
    const addNewData = async (data:any, image : File | null) => {
        ...
        const res = await fetchData("/contact", setFormLoading, "POST", formData);
        if (res?.success) {
    ...
    const handleEdit = (data : any, image : File | null) => {
        ...
        if (!data?.contact || !data?.email || !data?.address || (!data?.image && !image)) {
